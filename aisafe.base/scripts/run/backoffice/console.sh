#!/usr/bin/env bash
# Backoffice console (PostgreSQL). First run: add --bootstrap
set -euo pipefail
...
if [[ "${1:-}" == "--bootstrap" ]]; then
  ...
  echo "[info] Skipping bootstrap (use --bootstrap on first run)"
fi

echo "[run] AISafe console (config: $AISAFE_CONFIG)..."
